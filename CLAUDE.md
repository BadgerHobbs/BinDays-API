...
The same build step also provisions the `libcurl-impersonate` shared library (via `dart run dio_impersonate:install`) into `BinDays.Api.IntegrationTests/DartClient/.native/` (gitignored). The Dart/Dio transport loads it for councils behind a Cloudflare TLS-fingerprint challenge, presenting a real browser's TLS/HTTP-2 fingerprint. The C# harness only signals, per request, whether to enable impersonation (the `useImpersonate` test flag → `BINDAYS_IMPERSONATE` env var); all impersonation logic lives in the Dart client. Delete `DartClient/bin/send_request.exe` to force a recompile and re-provision.
