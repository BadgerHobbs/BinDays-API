...
RESUME_PROMPT="You stopped before completing all steps. This is a non-interactive CI/CD task with no user present.

Please verify and complete the implementation:
1. Check if $COLLECTOR_FILE exists - if not, create it
2. Check if $TEST_FILE exists - if not, create it
3. Check if .agent/playwright/out/${COLLECTOR_NAME}-screenshot.png exists
4. Run the verification commands from Phase 5.2 to confirm all files exist
5. Do NOT stop until you have confirmed all files exist on disk"
