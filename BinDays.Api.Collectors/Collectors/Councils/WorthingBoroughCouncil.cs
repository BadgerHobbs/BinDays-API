		...
		public GetBinDaysResponse GetBinDays(Address address, ClientSideResponse? clientSideResponse)
		{
			...
			else if (clientSideResponse.RequestId == 1)
			{
				...
				foreach (Match rawBinDay in rawBinDays)
				{
					...
					var matchedBinTypes = this.binTypes.Where(x => x.Keys.Any(y => collection.Contains(y)));
				...
				// Merge bin days collected on the same date
				var mergedBinDays = ProcessingUtilities.MergeBinDays(binDays);

				var getBinDaysResponse = new GetBinDaysResponse()
				{
					BinDays = mergedBinDays.AsReadOnly(),
					NextClientSideRequest = null
				};
